...
export function CustomerForm({
  ...
  const [state, formAction] = useActionState(
    initial ? updateCustomer : createCustomer,
    initialState,
  );
  const cancelHref = companyModulePath(companyId, "customers");

  return (
    <form action={formAction} className="space-y-5">
      <input type="hidden" name="company_id" value={companyId} />
      {initial ? <input type="hidden" name="customer_id" value={initial.id} /> : null}

      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-1.5">
          <Label>Kod</Label>
          <Input value={initial?.code ?? "MUS-01 otomatik"} disabled />
          {!initial ? (
